...
const BRAND = {
  ...
  gold: "#c9a646",  // brushed gold
  offwhite: "#f7f6f2",
  ink: "#111111",
};

// Your actual files in /public
const LOGO = "/Logo_LEXVOYAGE_transparent.png";
...
const Card = ({ children, className = "" }) => (
  <div className={`rounded-3xl border bg-white/90 backdrop-blur shadow-lg ${className}`} style={{borderColor:"rgba(255,255,255,.5)"}}>
    <div className="p-5 sm:p-6">{children}</div>
  </div>
);

function CTA({ href, onClick, children, variant = "primary", disabled = false }) {
  const base =
    "px-4 sm:px-5 py-3 rounded-xl font-semibold transition text-center inline-flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed";
  const style =
    variant === "primary"
      ? { backgroundColor: BRAND.green, color: "#fff", opacity: disabled ? 0.6 : 1 }
      : variant === "gold"
      ? { backgroundColor: BRAND.gold, color: "#111", opacity: disabled ? 0.6 : 1 }
      : { border: "1px solid #D1D5DB", backgroundColor: "#fff", color: "#111", opacity: disabled ? 0.6 : 1 };

  if (href) {
    return (
      <a href={disabled ? undefined : href} target="_blank" rel="noreferrer" className={base} style={style} aria-disabled={disabled}>
        {children}
      </a>
    );
  }
  return (
    <button onClick={disabled ? undefined : onClick} className={base} style={style} disabled={disabled}>
      {children}
    </button>
  );
}
...
const Note = ({ children }) => (
  <div className="flex items-start gap-2 text-sm rounded-lg px-3 py-2"
       style={{ background:"#FFFDF6", border:"1px solid #F5E7B2", color:"#7C5B0A" }}>
    <svg className="w-4 h-4 mt-0.5" viewBox="0 0 24 24" fill="currentColor">
      <path fillRule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25Zm.75 6a.75.75 0 1 0-1.5 0v6.5a.75.75 0 0 0 1.5 0V8.25ZM12 17a1 1 0 1 0 0 2 1 1 0 0 0 0-2Z" clipRule="evenodd"/>
    </svg>
    <span>{children}</span>
  </div>
);
...
export default function App() {
  ...
  const Nav = () => (
    <div className="flex items-center justify-between gap-3">
      <button className="px-4 py-2 rounded-xl border border-gray-300 hover:bg-gray-50 disabled:opacity-40"
      ...
      <div className="max-w-5xl mx-auto px-4 pt-8 pb-24 lg:pb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 space-y-5">
            {step === 1 && (
              <Card>
                ...
                <ol className="space-y-3 list-decimal pl-6 mt-4">
                  <li>Open the enrolment link <a className="underline text-blue-600" href={PLANNET_LINK} target="_blank" rel="noreferrer">plannetmarketing.com/alexy</a> <span className="ml-2"><CTA href={PLANNET_LINK} variant="outline">Open enrolment</CTA></span></li>
            ...
            {step === 3 && (
              <Card>
                ...
                <ol className="space-y-3 list-decimal pl-6 mt-4">
                  ...
                  <li>Initial the agreement boxes, then click <strong>Enrol Now</strong>.</li>
                </ol>
                <div className="mt-6"><Nav /></div>
              </Card>
            )}
            ...
            {step === 4 && (
              <Card>
                <StepHeader title="Final checks" subtitle="Confirm access to essentials before you celebrate." />
          ...
          <div className="space-y-5">
            ...
            <Card>
              ...
              <div className="grid gap-2">
                <CTA href={WHATSAPP_LINK}>WhatsApp me</CTA>
                <CTA href={`mailto:${CONTACT_EMAIL}`} variant="outline">Email: {CONTACT_EMAIL}</CTA>
                <CTA href={CALENDLY_LINK} variant="gold">Book a call (Calendly)</CTA>
              </div>
            </Card>
          </div>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 lg:hidden border-t border-gray-200 bg-white/90 backdrop-blur px-4 py-3 flex items-center justify-between">
          <button className="px-4 py-2 rounded-xl border border-gray-300 disabled:opacity-40"
                  onClick={() => setStep((s)=>Math.max(1,s-1))} disabled={step===1}>Back</button>
          <CTA onClick={next} disabled={!canProceed}>{step===total ? "Finish" : "Next →"}</CTA>
        </div>
