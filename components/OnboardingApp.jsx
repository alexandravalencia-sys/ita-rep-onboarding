...
function Card({ children, className = "" }) {
  return (
    <div className={`bg-white/80 backdrop-blur rounded-3xl shadow-lg border border-white/40 ${className}`}>
...
function CTA({ href, onClick, children, variant = "primary" }) {
  ...
  const styles = {
    ...
    gold: `bg-[${BRAND.gold}] text-black hover:opacity-90`,
  };
...
function CopyText({ text, label = "Copy" }) {
  ...
  return (
    <button
      onClick={async () => {
        try { await navigator.clipboard.writeText(text); setOk(true); setTimeout(() => setOk(false), 1600);} catch {}
      }}
...
function Collapsible({ title, children, defaultOpen = false }) {
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      <button onClick={() => setOpen((o) => !o)} className="w-full flex items-center justify-between gap-3 px-4 py-3 bg-gray-50 hover:bg-gray-100">
...
function SaveBar({ name, email, onClear }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center gap-2 text-sm">
      ...
      <button onClick={() => { onClear?.(); setSaved(true); setTimeout(() => setSaved(false), 1600); }} className="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
...
export default function App() {
  ...
  const Nav = () => (
    <div className="flex items-center justify-between gap-3">
      <button className="px-4 py-2 rounded-xl border border-gray-300 hover:bg-gray-50 disabled:opacity-40" onClick={() => setStep((s) => Math.max(1, s - 1))} disabled={step === 1}>← Back</button>
  ...
  return (
    <div className="min-h-screen text-gray-900" style={{ background: `radial-gradient(1000px 600px at 20% -10%, ${BRAND.offwhite}, #ffffff 50%), linear-gradient(120deg, ${BRAND.offwhite}, #ffffff)` }}>
      <div className="max-w-5xl mx-auto px-4 pb-28 pt-6 sm:pt-10">
        ...
        <div className="mb-6 sm:mb-10 flex flex-col items-center text-center">
          <img src={LOGOS.dark} alt="LEXVOYAGE" className="h-16 sm:h-20 w-auto mb-3" />
          <h1 className="text-3xl sm:text-4xl font-serif font-bold tracking-tight" style={{color: BRAND.green}}>Join LEXVOYAGE as an ITA & Rep</h1>
          <p className="mt-2 text-gray-600 max-w-2xl">A curated, step‑by‑step enrolment—clear, elegant, and mobile‑friendly.</p>
        </div>
        ...
        <div className="mb-8">
          <Card>
            ...
            <ul className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 list-none mt-3">
              {benefits.map((b, i) => (
                <li key={i} className="p-4 rounded-xl border border-gray-200 bg-white/70">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              <Card>
                ...
                <div className="mt-4 space-y-4">
                  <ol className="space-y-3 list-decimal pl-6">
                    <li>
                      Open the enrolment link <a className="text-blue-600 underline" href={PLANNET_LINK} target="_blank" rel="noreferrer">plannetmarketing.com/alexy</a> <CTA href={PLANNET_LINK}>Open enrolment</CTA>
                    </li>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <Collapsible title="Key links" defaultOpen>
                    <ul className="list-disc pl-6 space-y-2 text-sm">
                      <li>PlanNet enrolment: <a className="text-blue-600 underline" href={PLANNET_LINK} target="_blank" rel="noreferrer">plannetmarketing.com/alexy</a> <CopyText text={PLANNET_LINK} /></li>
          ...
          <div className="flex flex-col gap-5">
            <Card>
              {[1,2,3,4,5,6].map((n) => (
                <button key={n} onClick={() => setStep(n)} className="w-full flex items-center gap-3 mb-3 last:mb-0 text-left">
        ...
        <div className="fixed bottom-0 left-0 right-0 lg:hidden border-t border-gray-200 bg-white/90 backdrop-blur px-4 py-3 flex items-center justify-between">
          <button className="px-4 py-2 rounded-xl border border-gray-300" onClick={() => setStep((s) => Math.max(1, s - 1))} disabled={step === 1}>Back</button>
          <CTA onClick={() => setStep((s) => Math.min(total, s + 1))}>{step === total ? "Finish" : "Next →"}</CTA>
        </div>
